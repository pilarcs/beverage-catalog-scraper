...
def test_execucao_completa_com_api_falsa(tmp_path):
    ...
    codigo = entrada.main(
        raiz=_raiz(tmp_path), ambiente={"PILAR_COSMOS_TOKEN": "tk"}, fabrica_cliente=fabrica, instalar_sinais=False
    )
    assert codigo == 0
...
def test_le_token_do_arquivo_env_quando_ambiente_nao_e_passado(tmp_path, monkeypatch):
    raiz = _raiz(tmp_path)
    monkeypatch.delenv("PILAR_COSMOS_TOKEN", raising=False)
    ...
    (raiz / ".env").write_text("PILAR_COSMOS_TOKEN=do-arquivo\n", encoding="utf-8")
