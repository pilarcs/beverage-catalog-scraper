...
def test_gravar_pagina_cria_arquivo_com_coleta_e_resposta(tmp_path):
    ...
    caminho = bruto.gravar_pagina(tmp_path, "22030000", 2, URL, MOMENTO, corpo, "PILAR")
    assert caminho == tmp_path / "ncm_22030000" / "p0002_20260916T061705Z.json"
    assert json.loads(caminho.read_text(encoding="utf-8")) == {
        "coleta": {
            "fonte": "ncm:22030000", "ncm": "22030000", "pagina": 2, "url": URL,
            "coletado_em": "2026-09-16T06:17:05Z", "responsavel": "PILAR",
        },
        "resposta": corpo,
    }
...
def test_mesma_pagina_no_mesmo_segundo_nao_sobrescreve(tmp_path):
    primeiro = bruto.gravar_pagina(tmp_path, "22030000", 2, URL, MOMENTO, resposta("22030000", 2, 200, []), "PILAR")
    segundo = bruto.gravar_pagina(tmp_path, "22030000", 2, URL, MOMENTO, resposta("22030000", 2, 201, []), "PILAR")
...
def test_listar_paginas_em_ordem_cronologica_e_ignora_temporarios(tmp_path):
    ...
    bruto.gravar_pagina(tmp_path, "22085000", 1, URL, depois, resposta("22085000", 1, 1, []), "PILAR")
    bruto.gravar_pagina(tmp_path, "22030000", 1, URL, MOMENTO, resposta("22030000", 1, 1, []), "PILAR")
